...
const headCells = [
  ...
  { id: 'shipper', numeric: false, disablePadding: false, label: 'SHIPPER' },
  { id: 'line', numeric: false, disablePadding: false, label: 'LINE' },
  ...
  { id: 'status', numeric: false, disablePadding: false, label: 'COMPLETED?' },
];
...
export default function ConsignTable({consignments}) {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = consignments.map((n) => n.container);
  ...
  const handleClick = (event, container) => {
    const selectedIndex = selected.indexOf(container);
    let newSelected = [];

    if (selectedIndex === -1) {
      newSelected = newSelected.concat(selected, container);
  ...
  const isSelected = (container) => selected.indexOf(container) !== -1;

  const emptyRows = rowsPerPage - Math.min(rowsPerPage, consignments.length - page * rowsPerPage);
            ...
            <TableBody>
              ...
              {stableSort(consignments, getComparator(order, orderBy))
                ...
                .map((consignment, index) => {
                  const isItemSelected = isSelected(consignment.container);
                  const labelId = `enhanced-table-checkbox-${index}`;

                  return (
                    <StyledTableRow
                      hover
                      onClick={(event) => handleClick(event, consignment.container)}
                      role="checkbox"
                      aria-checked={isItemSelected}
                      tabIndex={-1}
                      key={consignment.container}
